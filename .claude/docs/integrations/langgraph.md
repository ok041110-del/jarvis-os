...
> **복원 노트 (Traceability, 2026-09-02)**
>
> 이 문서는 커밋 `a7fd7c5`("LangGraph 도입 검증 Evidence 추가")로 처음
> 작성되었으나 그 커밋이 담긴 브랜치(`origin/claude/context7-graphify-validation-g3raoe`)가
...
> 하위 호환을 유지함을 보인다. E2는 그 이상을 보이지 않는다 —
> Conditional Edge/Loop을 Jarvis 도메인 node·State로 검증한 것이 아니고,
> `ADC-0019` A-IN(e)의 "호출자가 Checkpoint 값을 보관"하는 소유 모델을
> 검증한 것도 아니며(E2는 LangGraph가 소유하는 `MemorySaver`를 사용),
> v2 맥락의 Reversibility나 "병렬 + 조건부" 상호작용을 검증하지 않았다.
>
> **이 복원은 Governance 결정을 바꾸지 않는다.** `RFC-0019`/`ADC-0019`/
> `ADR-0008`의 Architecture·Governance 판단, `BASELINE.md` v1.12,
> `IMPLEMENTATION_RULES.md`는 이 복원으로 전혀 변경되지 않으며, 이
> 문서의 존재가 LangGraph 채택을 승인하지 않는다(`ADC-0019` §Decision
> 조건 6, §Q8). 본문 Summary 두 번째 항목("v2 BASELINE에 배선할 자리가
> 없다")은 2026-08-30 시점 서술이다 — 이후 `ADC-0019`/`ADR-0008`이
> §16.6 Scoped Workflow Graph Execution boundary의 **존재만**
> Accept(Scoped, Conditional)했고, 구현체 선택은 여전히 미결이다.
>
> **"Evidence가 `main`에 존재한다" ≠ "Evidence가 충분하다".** 이 복원은
> 전자만 해결한다. `ADC-0019` §Q2는 Evidence가 2건·동일 계보(LangGraph)로
> Governance v2 Rule B(3건 이상 독립 관찰)를 형식적으로 충족하지 못함을
> 이미 명시하고 있으며, 이 복원은 그 판단에 영향을 주지 않는다.
