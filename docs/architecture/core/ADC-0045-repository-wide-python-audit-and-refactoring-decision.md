...
|---|---|
| ID | ADC-0045 |
| Status | ADOPT — Governance Lifecycle Accepted, Implementation은 별도 승인 필요 |
| Owner / Scope | `RFC-0042-repository-wide-python-audit-and-refactoring-governance.md`가 설계한 Lifecycle(Inventory → Deterministic Audit → Semantic/Ponytail Review → Candidate Classification → Refactoring Wave → Validation → Evidence), Ponytail 권한 경계, 7종 Candidate Classification, Wave 정책, Validation 정책을 공식 Governance Decision으로 채택할지 판정 |

## 2. Context & Problem

이 ADC는 `RFC-0042`의 조사 내용을 재론하지 않고 그 결론 위에서
ADOPT/KEEP(=현행 유지)/REJECT 중 하나를 판정한다.
...
Q4가 기록한 Wave 크기 상한 미고정 Gap은 위 Decision의 "조건" 항목으로
보완한다 — 별도 Risks 섹션은 원문에 없으며 신설하지 않는다.
...
| Type | Path |
|---|---|
| RFC | `docs/architecture/core/RFC-0042-repository-wide-python-audit-and-refactoring-governance.md` |
| Precedent | `docs/architecture/core/RFC-0037-stage04-multi-agent-ponytail-boundary.md`, `docs/architecture/core/ADC-0040-stage04-multi-agent-ponytail-decision.md`(Ponytail 원 정의, 재론하지 않음) |
| Rule | `hqs/development/IMPLEMENTATION_RULES.md` |
| Evidence(별도 Workstream, main 미병합) | `docs/research/STAGE-TO-TEAM-MIGRATION-ARCHIVE-CLEANUP-INVESTIGATION-0001.md`, `docs/research/ARCHIVE-CLEANUP-REVALIDATION-0001.md` |

### Change History

| Date | Change | Reason |
|---|---|---|
| — | 최초 작성 | RFC-0042 Repository-wide Python Audit & Refactoring Governance Decision(ADOPT) |
| 2026-09-22 | 6-섹션 템플릿으로 구조 보존형 재배치(Identity & Status 표 추가, 헤딩 레벨 조정, Related를 표 형식으로 재정리) — Q1~Q6·Decision 조건 전부 verbatim 유지, `ARCHIVE-CLEANUP-REVALIDATION-0001.md` 인용은 원문 그대로 보존(별도 Workstream 미병합 상태, 이 정규화가 병합 여부를 바꾸지 않음), 압축 없음 | Batch 6 정규화 |
