...
### 3.4 위 8개 패턴 Evidence의 수준 — 무엇을 증명하고 무엇을 증명하지 않는가

§3.3의 8개 이상 ad hoc 패턴은 **관찰된 사실**(observation)이다. 다음은
그 관찰이 **닫지 않는 질문**을 명시적으로 분리한 것이다 — 이 구분이
없으면 현황 조사가 은연중에 Architecture 변경의 근거로 격상될 위험이
있다.

- 이 Evidence가 보여주는 것: **현재 코드 구조**와, H-6이 확정될 경우
  **잠재적 Migration 후보가 될 수 있는 지점들의 목록**뿐이다.
- 이 Evidence가 **단독으로 증명하지 않는 것**: 공통 Segment 구조가
  필요하다는 Architecture 변경의 필요성. 8개 패턴이 존재한다는 사실
  자체는 `ARCHITECTURE_GOVERNANCE.md`의 ADC 채택 기준("지금 결정하지
  않으면 진행 불가" / "되돌리는 비용이 커진다") 중 어느 것도 자동으로
  충족시키지 않는다.
- **실제로 확인되지 않은 것**: 이 8개 패턴 중 어느 것에서도 순서
  불일치, Identifier 부재·충돌, 또는 그로 인한 **실제 장애·운영
  실패 사례는 관찰되지 않았다**(0건). "독립적으로 재발명됐다"는
  사실과 "그로 인해 문제가 발생했다"는 사실은 다르며, 이 RFC는 후자를
  주장하지 않는다.
- **별도 검토가 필요한 것**: 공통 Segment 구조를 도입했을 때의
  비용(마이그레이션 작업량, §8·§9)과 효과(순서·Identifier 보장의
  실질적 이득)는 이 RFC가 계량하지 않는다 — 후속 ADC 단계에서
  별도로 다뤄야 할 사항이다.
- **관찰 vs 해석·제안·미확인의 분리**: §3.3의 각 bullet은 grep/코드
  읽기로 직접 확인한 **관찰**이다. §4~§9의 옵션·기준·trade-off는 그
  관찰로부터 이 RFC가 구성한 **제안**이며, 관찰이 아니다. 이 RFC의
  어느 부분도 "8개 패턴이 있으므로 H-6을 확정해야 한다"는 결론을
  내리지 않는다 — 결론은 §11에서 Owner에게 넘긴다.
...
H-5(Identifier 파생)와 H-6(Segment 자료구조)은 서로 다른 미결
항목이지만 완전히 독립적이지 않다. 아래 구분은 **관계를 설명할
뿐이며, 이 설명 자체가 H-5나 H-6 중 어느 것도 확정하지 않는다.**

- **공동으로 결정해야 하는 사항**: Identifier가 Segment 구조체의
  한 **필드**로 존재한다는 사실(§13.1 Model 자체는 이미 확정)은
  H-6이 어떤 옵션(H6-A/B/C)을 택하든 유지되지만, **H5-B(내용 기반
  해시 파생)를 택할 경우 그 해시가 계산되는 대상(Content, 필요하면
  Source)이 H-6이 정의하는 필드 경계와 정확히 일치해야 한다** —
  즉 H5-B와 H6-A/C는 "무엇을 해시할 것인가"를 공유해서 결정해야
  하는 관계다.
- **독립적으로 결정 가능한 사항**: H5-A(호출자 주입)는 H-6의 어느
  옵션과도 독립적으로 결정할 수 있다 — 호출자가 이미 갖고 있는
  Identifier 값을 어떤 자료구조(H6-A/B/C)에 담든 주입 방식 자체는
  바뀌지 않는다. 반대로 H6-B(마커 기반 델리미터)는 H-5의 어느
  옵션과도 독립적으로 결정할 수 있다 — 델리미터 파싱 방식은
  Identifier가 주입되든 파생되든 동일하다.
- **한 결정이 다른 결정에 미치는 영향**: H6-A/C(명시적 구조체)를
  택하면 H5-B(해시 파생)의 "무엇을 해시하는가" 질문에 답하기
  쉬워진다(필드가 명시적이므로) — 반대로 H6-B(델리미터 방식)에서
  H5-B를 택하면 "Content의 경계"가 델리미터 파싱에 의존하게 되어
  해시 계산의 결정론(A-5)이 파싱 규칙의 정확성에 종속된다는 새
  위험이 생긴다. 즉 **H6-B + H5-B 조합은 H6-A/C + H5-B 조합보다
  결합 위험이 크다** — 이것은 관찰된 사실이 아니라 이 RFC가
  §4·§5의 옵션 정의로부터 도출한 논리적 귀결이다.
- **하나의 ADC로 묶을지 분리할지**: 위 의존관계 때문에 H-5/H-6을
  하나의 ADC로 묶는 편이 자연스러워 보일 수 있으나, `ADC-0016`
  §16.3~§16.6이 보여준 것처럼 서로 연관된 항목도 개별 Boundary
  Question으로 분리해 순차 처리한 선례가 있다. **이 RFC는 어느
  쪽이 옳은지 판단하지 않는다** — §11(Owner에게 요청할 결정
  사항)에 이미 있는 항목 그대로 남긴다.
...
### 8.1 Stable Prefix 검증과 "Segment 재사용"의 관계 — 명확화

§1과 위 항목은 B 검증에 "재사용 지점"이 필요하다고 말했으나, 이것이
**동일 Segment 객체를 그대로 재사용해야 한다는 뜻으로 읽혀서는
안 된다.** 다음을 구분한다 — 아래는 전부 **제안 또는 설계
선택지**이며, 어느 것도 확정된 Contract가 아니다.

- **동일 Segment 객체의 재사용**은 B를 검증하는 가능한 구현 방식
  **중 하나**일 뿐이다(예: 캐시된 Segment 인스턴스를 여러 호출에
  그대로 전달).
- **Segment 객체 재사용이 B 검증에 반드시 필요한지는 이 RFC가
  결정하지 않는다** — 이것은 H-6이 확정될 때 함께 정해질 설계
  대상이다(§5, §5.1).
- **대안 선택지**: 매 호출마다 Segment를 **새로 생성**하더라도,
  동일한 Context Source와 동일한 Ordering Policy(순서 규칙)로부터
  나온 결과물이라면, 그 결과로 만들어진 Prefix들을 **값으로
  비교**하는 방식으로도 B를 검증할 수 있다 — 이 경우 "재사용"은
  객체 동일성(identity)이 아니라 **구조적 동일성(structural
  equality)**의 문제가 된다. 이는 §13.3 A-5("같은 입력 + 같은
  Policy → 같은 Context")와도 자연스럽게 정합한다.
- 이 구분은 OQ-2(§10)와 직접 연결된다 — 어느 방식을 택할지는
  H-6 확정 이후 별도로 판단할 사항이며, 이 RFC는 두 방식이 모두
  **선택지로 존재한다**는 사실만 기록한다.
...
### 12.1 Review Amendment 반영 (추가, 원 Self Review 미변경)

이전 검토에서 확인된 3개 보완 사항을 additive하게 반영했다 — 기존
§0~§11 원문은 삭제·수정하지 않았다.

1. **Stable Prefix와 Segment 재사용 관계 명확화** — §8.1 추가. 객체
   재사용과 구조적 동일성 비교를 별개 선택지로 분리했고, 어느 쪽이
   B 검증에 필요한지는 확정하지 않았다.
2. **H-5/H-6 의존관계 명확화** — §5.1 추가. 공동 결정 사항·독립
   결정 사항·상호 영향·ADC 분리 여부를 구분했으며, 이 설명 자체가
   H-5·H-6을 확정하지 않는다고 §5.1 서두에 명시했다.
3. **8개 ad hoc 패턴 Evidence 수준 제한** — §3.4 추가. 현황
   Evidence와 Architecture 변경 필요성의 단독 증거를 분리했고,
   실제 장애·순서 불일치·Identifier 부재로 인한 운영 실패가
   0건임을 명시했으며, 비용·효과 검토를 별도 과제로 남겼다.

이 세 추가 이후에도 Status는 **Proposed**, H-5·H-6은 **Defer/미결**
그대로이며, B가 해소됐다는 표현은 어디에도 추가하지 않았다.
