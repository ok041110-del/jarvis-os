# RFC-0001: Jarvis OS Core Baseline

**Status**: Proposed (검토 대상, 결정 아님)
**Author**: Claude Code (Development HQ Phase 1 종료 → Core 설계 단계 진입 시점 요청에 대한 RFC)
**대상**: Jarvis OS Core의 Mission, Boundary, Core Module, Core API, Design Principle (Baseline 정의만)
...
> Architecture Baseline v1.0의 Concept Model만 Core 수준으로 일반화한다.

## 0. 이 RFC가 열린 이유

Jarvis OS는 HQ 구현(Development HQ MVP-0001~0013) 단계에서 Core 설계 단계로
...
실제로 관찰·결정된 사실들을 Jarvis OS Core의 Baseline으로 일반화하는 것이다.

## 1. Core Mission

Jarvis OS Core는 모든 HQ가 공통으로 사용하는 Shared Infrastructure다.

- Core는 도메인 로직을 갖지 않는다.
- Core는 공통 서비스를 제공한다.
- HQ는 Core를 사용한다.
- Core는 HQ를 알지 못한다.
...
Core는 HQ 위에 존재하지 않는다. Core는 HQ 아래에도 존재하지 않는다. Core는
모든 HQ가 공유하는 공통 계층이다.
...
이 RFC는 그 계층의 "Jarvis OS" 위치를 "Core"로 명명하고, 지금까지 이름 없이
Out of Scope로 남아 있던 자리(`BASELINE.md` §10: Kernel Architecture,
Component Design)를 Core라는 하나의 층으로 일반화할 뿐, 계층 구조 자체를
새로 발명하지 않는다.

## 3. Boundary

Core는 Service를 제공한다. HQ는 Capability를 제공한다.

Core는 도메인을 모른다. HQ는 도메인을 안다.

Core는 Specification, Context, Execution Result, Event 같은 공통 Artifact만
다룬다.
...
실행 메커니즘을 지금 Kernel/Core 수준에서 어떻게 일반화할지는 여전히
Open이다(`docs/03_adc/ADC.md` ADC-02 "Runtime 개념의 존폐", 우선순위 NOW).
이 RFC는 Workflow가 Core Module이라는 위치만 확인하고, 그 내부 구조(Runtime
명칭 포함)는 결정하지 않는다.
...
Memory가 HQ 내부에서 자체 구현되지 않고 Core 수준의 공통 서비스 자리로
...
그 질문에 답하지 않고, Memory가 Core Module이라는 위치만 확인한다.
...
따라서 Execution Layer를 Core의 일부로 정의하는 것은 새 결정이 아니라, 이미
확정된 System Boundary와 RFC-0005의 관찰을 하나의 Core Module로 명명하는
...
Model에 이미 존재했던 자리를 Core Module로 재확인한 것이지, Development
HQ Observation으로 새로 검증된 것이 아니다.

## 5. Core API Principle

Core는 구체 구현이 아니라 Service Interface만 노출한다.
...
- **HQ Independent** — Core는 HQ를 알지 못한다(§1, §3 근거와 동일).
...
- **Artifact Driven** — RFC-0005 §3이 정리한 것처럼, Core Module 사이의
  전달 단위는 항상 명세된 Artifact(Implementation Specification 8개 항목
  같은)였지, 코드나 프로세스가 아니었다.
- **Service Oriented** — §5의 Core API Principle과 동일. Core는 Interface만
  노출한다.
...
- 이 RFC는 Core Module을 어떻게 구현할지 논의하지 않는다.
...
ADR
↓
Architecture Baseline Update (Core 절 신설)
```
