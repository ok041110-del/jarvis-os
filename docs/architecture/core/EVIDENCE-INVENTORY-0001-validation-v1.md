...
> §8의 "§15.1 경계표의 범위를 진술하는 문제"가 실행되었다. `BASELINE.md`
> §15.1 경계표 바로 뒤에 "위 표는 ... 경계를 넘는 모든 항목의 전수
> 목록이 아니다"라는 문장이 추가되어, 표가 예시이며 전수 목록이
> 아님을 명시했다. 이것은 N-3(§6)이 요구한 Observation을 만든 것이며,
> **R-1("경계표에 행이 없다 = 결손이다")의 전제를 약화시킨다** — 표가
> 애초에 전수를 주장한 적이 없다는 점이 이제 문서에 명시적으로
> 드러났기 때문이다. 그러나 이 Amendment는 **V-1의 심각도 프레이밍만
> 완화**할 뿐, GAP-1(Context-level Identifier·Metadata의 실제 입력
> 경로 미정)을 해결하지 않는다. N-1·N-2는 여전히 Kernel 구현·사용
> 시점에만 관찰 가능한 상태로 남는다.
