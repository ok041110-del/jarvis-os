...
> **Amendment(V-1 severity framing 완화, 2026-09-20, `BASELINE.md`
> §15.1 추가 문장, `EVIDENCE-INVENTORY-0001` 참조)**: §15.1 경계표
> 바로 뒤에 그 표가 "경계를 넘는 모든 항목의 전수 목록이 아니다"라는
> 문장이 추가되었다. 이는 위 표의 "호출자가 주입 → §15.1 경계표의
> 외부 입력 3개에 없다 → 표가 불완전해진다"는 문제 제기의 전제(표가
> 전수 목록이라는 것)를 제거한다. **그러나 이 Amendment는 Context
> 수준 Identifier·Metadata가 실제로 어디서 오는지는 여전히 정의하지
> 않는다** — 세 후보(호출자 주입/Segment 파생/Kernel 생성) 중 어느
> 것도 아직 채택되지 않았다. 이 Amendment가 완화하는 것은 "표 자체의
> 불완전성"이라는 부차적 문제일 뿐, 위 "영향" 문단의 "Component RFC가
> 이 질문에 부딪힌다"는 핵심 결손(GAP-1)은 그대로 남는다.
...
> **Amendment(V-2 문서적 결손 해소, 2026-09-19, `BASELINE.md` commit
> `9d1b592`)**: §13.2에 "병합 판정은 Segment가 도착한 순서에 의존하지
> 않는다"는 문장이 추가되어, 위에서 "미진술"이라 지적한 성질이 이제
> **Baseline 문언에 명시**됐다. **이 Amendment가 해소하는 것은 문서적
> 결손뿐이다** — 이 성질이 실제 Kernel 구현에서 실행으로 검증된 것은
> **아니다**(Kernel Context Builder 자체가 아직 구현되지 않았다, §10
> Out of Scope). 즉 "Baseline이 이 성질을 요구한다는 사실"은 확정됐지만
> "구현이 이 성질을 실제로 만족한다는 사실"은 여전히 검증 대상이 아니다
> (Kernel 구현 착수 이후에만 검증 가능). 위 판정("조건부 Pass")과 그
> 근거 서술은 발견 당시 상태로 그대로 보존한다.
...
> **Amendment(V-6(a) 문서 명확화, 2026-09-20, `BASELINE.md` §13.2
> 추가 문장)**: V-6은 두 하위 질문으로 나뉜다 — **(a)** 0-Segment
> Context가 §13.1·§13.2 정의와 논리적으로 양립하는가, **(b)** ⑥Render가
> 0-Segment Context를 어떻게 출력하는가(빈 출력/오류 등). `BASELINE.md`
> §13.2에 "Segment가 0개인 Context도 유효하다"는 문장이 추가되어
> **(a)만 해소**했다 — §13.2의 검증 규칙(Identifier 존재·유일성, Source
> 존재, Order Key 비교 가능성)이 Segment의 존재를 전제하지 않으며,
> §13.1의 "유한한 열"이 길이 0을 배제하지 않는다는 점을 명시한 것이다.
> **(b)는 이 Amendment로 결정되지 않았다** — Renderer의 빈 Context
> 출력 방식은 여전히 H-4(Renderer 내부 구현)로 남아 있으며,
> Implementation-Dependent 상태다. 이 표의 V-6 행은 (a)만을 가리키는
> 것으로 갱신되며, (b)는 별도 후속 항목으로 추적한다.
