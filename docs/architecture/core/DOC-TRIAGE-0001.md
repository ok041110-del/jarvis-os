...
> **Amendment(N-3 실행, 2026-09-20, `BASELINE.md` §15.1 추가 문장)**:
> 위 "최소 수정 범위"가 제안한 N-3(경계표의 범위 진술)이 사용자 승인을
> 거쳐 실행됐다. §15.1 경계표 바로 뒤에 "위 표는 ... 경계를 넘는 모든
> 항목의 전수 목록이 아니다"라는 문장이 추가됐다. **이것은 D-1이 지적한
> 문제의 심각도 프레이밍만 완화**한다 — 표가 전수 목록이라는 전제가
> 제거되어 "Context 수준 Identifier·Metadata가 표에 없다 = 결손"이라는
> 해석의 근거가 약해진다. **그러나 D-1의 근본 사실(§13.1의 두 요소가
> 여전히 어떤 규칙에도 소비되지 않는다는 것, GAP-1)은 이 Amendment로
> 해결되지 않는다.** N-1·N-2는 여전히 Kernel 구현·사용 시점에만
> 관찰 가능한 상태로 남는다. 원 표는 발견 당시 상태 그대로 보존한다.
...
> **Amendment(문서적 결손 해소, 2026-09-19, `BASELINE.md` commit
> `9d1b592`)**: 위 "최소 수정 범위"가 제안한 문장("병합 판정은
> Segment가 도착한 순서에 의존하지 않는다")이 사용자 승인을 거쳐
> §13.2에 실제로 추가됐다. **D-2가 지적한 문서적 결손(성질은 성립하나
> 진술되지 않음)은 이제 해소됐다.** 이 Amendment는 **문서 수준의
> 해소만** 확정한다 — 위 "실제 저장소 사실" 행이 근거로 삼은 논리적
> 분석(중복 제거·오류 판정 각각의 순서 무관성)은 그대로 유지되며,
> **실제 Kernel 구현에서 이 성질이 실행으로 검증된 것은 아니다**
> (Kernel Context Builder 미구현, §10 Out of Scope). 원 표의 각 행은
> 발견 당시 상태 그대로 보존한다.
