...
합계 **164건**(TEMPLATE/README/CANDIDATES 제외) — 이전 라운드 보고서가
사용한 "163건"과 1건 차이가 있다(재계산 시점 차이로 추정, 재조사
필요 — §9 참조). 이번 계획은 오늘 시점 실제 파일시스템 재확인 값인
164건(= 이미 처리 10건 + 이번 계획 대상 154건)을 기준으로 한다.
...
1. **164 vs 163 불일치**: 오늘 재계산한 전체 RFC/ADC/ADR 파일 수(164)가
   PR #214 이전 라운드 보고서의 "163건"과 다르다. 어느 시점에 파일이
   추가/제외됐는지 재확인이 필요하다 — 이번 계획의 결론에는 영향이
   없으나(이미 처리 10건 + 이번 계획 154건 = 164건으로 자체 정합),
   원장(`docs/decisions/{rfc,adc,adr}.md`)의 등록 총계와도 재대조가
   필요하다.
...
**Batch 1(Low, 2건: `docs/decisions/adr/ADR-0007-baseline-relocation.md`,
`docs/architecture/core/ADC-0026-gate-c-real-engine-partial-discharge.md`)를
가장 먼저 진행할 것을 권장한다.** 이유:

- 위험도가 가장 낮아(외부 실제 인용 1건뿐) 기법 A(전체 압축)를
  안전하게 다시 검증할 수 있는 저비용 시험대다.
- 두 건 모두 서로 다른 트리(DevHQ-ADR, Kernel-ADC)에 속해 있어,
  Batch 1 통과 시 두 트리 각각에서 기법 A가 유효함을 동시에
  확인할 수 있다.

Batch 1 승인과 실행 결과 검증 이후, Batch 2(DevHQ-RFC Medium 6건)로
순차 진행하는 것을 제안한다 — 이미 §5의 배치 순서를 따른다.
