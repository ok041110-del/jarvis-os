...
def _extract_section_body(text: str, section: str) -> str:
    ...
    다음 `## ` 마커 전까지를 본문으로 본다. `Reference Design`은 중첩된 하위 문서를 포함하므로 예외적으로 텍스트 끝까지를 본문으로 취급한다."""
    marker = f"## {section}\n"
...
`RENDERING_MAP`에 따라 재배치할 뿐 본문 텍스트는 그대로 옮긴다 (Deterministic).
    """
...
`## Validation Notes` 같은 하위 소제목과 구분하기 위해 줄 시작이 정확히 `# ` 하나인 경우만 인정한다. Artifact Mapping 검증용 보조 함수이며 `build_prompt_specification()`의 변환 경로에는 관여하지 않는다.
    """
    return {
