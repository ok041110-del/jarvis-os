"""MVP-0009: collect_relevant_context()를 build_context_bundle() 수준의
구조화된 Context Bundle로 발전시키고, Planning이 그 Context Bundle만
입력으로 사용했을 때의 결과를 관찰한다.

새 Runtime/Task Dispatcher/Stage Runner/Pipeline Runner/Capability를
추가하지 않는다. Planning Capability(`requirements_agent_requirement_analysis`,
MVP-0004)의 내부 로직은 수정하지 않는다 — `engine.py`의
`_analyze_requirement`가 이미 `description`에서 `[Relevant Context]`
마커 이후를 통째로 `Reference Context`로 취급하는 계약을 그대로
재사용한다(MVP-0005). 이번 MVP는 그 마커 뒤에 들어가는 *내용의 구조*만
바꾼다 — 카테고리별 파일 목록(flat)에서 8개 항목(Issue/Goal/Relevant
Documents/Relevant Code/Relevant Observations/Relevant Decisions/Known
Constraints/Open Questions)으로.

같은 Issue에 대해 기존 방식(`run_issue_to_planning`, MVP-0005의 flat
Context)과 새 방식(Context Bundle)으로 각각 Planning을 실행해 두
결과를 나란히 반환하는 `run_comparison()`만 추가한다 — Context 품질
차이가 Planning 결과에 어떤 영향을 주는지 관찰하기 위함이며, 어느
쪽이 더 나은지는 이 코드가 판단하지 않는다.
"""
