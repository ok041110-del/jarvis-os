...
def composition_analyst_composition_analysis(data: str) -> str:
    instruction = (
        ...
        "the ETF's tracked index and weighting methodology, write an analysis "
        "in prose: what index it tracks, how constituents are weighted, and "
        "what recent methodology changes imply. Do not give a buy/sell "
...
def holdings_analyst_holdings_analysis(data: str) -> str:
    instruction = (
        ...
        "inconsistencies across sources. Do not give a buy/sell "
        "recommendation."
    )
...
def cost_analyst_cost_analysis(data: str) -> str:
    instruction = (
        ...
        "ratio and tracking-error data, write an analysis in prose of the "
        "fund's cost structure and how well it likely tracks its benchmark. "
        "If tracking-error data is missing, say so explicitly rather than "
        "estimating it. Do not give a buy/sell recommendation."
    )
...
def performance_analyst_performance_analysis(data: str) -> str:
    instruction = (
        ...
        "profile. Note any missing quantitative volatility metrics "
...
def distribution_analyst_distribution_analysis(data: str) -> str:
    instruction = (
        ...
        "profile: yield, payment frequency, and recent distribution trend. "
...
def macro_analyst_macro_analysis(data: str) -> str:
    instruction = (
        ...
        "market-scenario data, write an analysis in prose of the "
        "macroeconomic backdrop and its plausible range of outcomes for this "
        "fund. Treat any scenario probabilities as reported estimates, not "
        "facts. Do not give a buy/sell recommendation."
    )
...
def report_writer_final_report(
    ...
    instruction = (
        "Write a final research report for QQQ (Invesco QQQ Trust) that "
        "integrates all sections below into one coherent document with "
        "clear headers (Composition, Holdings, Cost, Performance, Exposure, "
