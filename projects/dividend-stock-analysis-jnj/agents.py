...
와 코드를 공유하지 않는 별도의 project-local 구현이다(첫 Dividend Stock
Dogfooding — Dividend Stock Team/Agent를 이 파일에서 선행 설계하지
않는다).
...
def report_writer_final_report(
    ...
    instruction = (
        "Write a final research report for JNJ that integrates all sections "
