...
이 모듈은 Kernel/HQ 어느 코드도 import하지 않는다. 표준 라이브러리만 쓴다. `RFC-0024`(Agent Domain, Defer)의 Identity/Role/Capabilities 필드를 이 실험이 확정하는 것이 아니다 — 각 함수는 그 필드들과 닮은 매개변수를 받을 뿐, Kernel/HQ에 등록되는 Domain 객체가 아니다.
"""
