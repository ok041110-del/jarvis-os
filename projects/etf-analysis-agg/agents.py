...
쓴다. `projects/etf-analysis-{qqq,schd}`와 코드를 공유하지 않는 별도의
project-local 구현이다(세 번째 ETF Dogfooding, 채권형 — ETF Team/Agent를
이 파일에서 선행 설계하지 않는다).

QQQ/SCHD의 7개 분석(Composition/Holdings/Cost/Performance/Exposure/
Distribution/Macro)을 이번에는 사용자 지시에 따라 6개로 재구성한다:
Composition/Index, Holdings·Exposure(통합), Cost·Tracking(통합),
Performance·Risk(통합), Distribution, Macro. 이 재구성 자체가 "역할
경계를 다르게 그어도 동일한 Development HQ 패턴이 그대로 동작하는지"를
관찰하는 대상이다.
"""
...
def composition_analyst_composition_analysis(data: str) -> str:
    instruction = (
        ...
        "the ETF's tracked index and portfolio-construction method (e.g. "
        ...
        "fund's behavior. Do not give a buy/sell recommendation — that is "
        "not your role."
    )
...
def holdings_exposure_analyst_holdings_exposure_analysis(data: str) -> str:
    instruction = (
        ...
        "is not a meaningful concept for this fund (e.g. a broadly "
        "diversified bond fund), say so explicitly and focus on the "
        "sector/credit/maturity structure instead. Do not give a buy/sell "
        "recommendation."
    )
...
def performance_risk_analyst_performance_risk_analysis(data: str) -> str:
    instruction = (
        ...
        "(including interest-rate sensitivity if duration data is given). "
        "Note any missing or vague quantitative claims explicitly. Do not "
...
def distribution_analyst_distribution_analysis(data: str) -> str:
    instruction = (
        ...
        "reported yield figures. Do not give a buy/sell recommendation."
    )
...
def macro_analyst_macro_analysis(data: str) -> str:
    instruction = (
        ...
        "profile. If the data contains conflicting or unverified reports, "
...
def report_writer_final_report(
    ...
    instruction = (
        "Write a final research report for AGG (iShares Core U.S. Aggregate "
        "Bond ETF) that integrates all sections below into one coherent "
        "document with clear headers (Composition, Holdings/Exposure, Cost/"
        "Tracking, Performance/Risk, Distribution, Macro, Bull Case, Bear "
        "Case, Synthesis). End with an explicit disclaimer that this is an "
