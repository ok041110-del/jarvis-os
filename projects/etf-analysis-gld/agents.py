...
def composition_analyst_composition_analysis(data: str) -> str:
    instruction = (
        ...
        "full replication vs. sampling), write an analysis in prose of what "
        "index it tracks and how that construction method could affect the "
        "fund's behavior. If the fund does not track an index at all (e.g. it "
        "tracks a physical commodity's spot price instead), say so explicitly "
        "and describe what it tracks instead. Do not give a buy/sell "
        "recommendation — that is not your role."
    )
...
def holdings_exposure_analyst_holdings_exposure_analysis(data: str) -> str:
    instruction = (
        ...
        "sector/asset-type breakdown, credit-quality distribution, and "
        "maturity distribution, write an analysis in prose of the fund's "
        "concentration and exposure profile. If single-name concentration, "
        "sector, credit-quality, or maturity are not meaningful concepts for "
        "this fund (e.g. a fund holding a single physical commodity), say so "
        "explicitly and describe what the fund's actual exposure structure "
        "is instead. Do not give a buy/sell recommendation."
    )
...
def cost_tracking_analyst_cost_tracking_analysis(data: str) -> str:
    instruction = (
        ...
        "(an index, or, if the fund has no index, the underlying asset's spot "
        "price), including any stated reasons the construction method itself "
        "might cause tracking difference. If a numeric tracking-error figure "
        "is missing, say so explicitly. Do not give a buy/sell recommendation."
    )
...
def performance_risk_analyst_performance_risk_analysis(data: str) -> str:
    instruction = (
        ...
        "yield, duration, and volatility/risk data, write an analysis in "
        "prose of the fund's historical performance and its risk profile "
        "(including interest-rate sensitivity if duration data is given, or "
        "commodity price risk if that is the dominant risk instead). If "
        "standard equity/bond risk metrics (beta, duration) do not apply to "
        "this fund's asset class, say so explicitly rather than forcing them. "
        "Note any missing or inconsistent quantitative claims explicitly. Do "
...
def distribution_analyst_distribution_analysis(data: str) -> str:
    instruction = (
        ...
        "say so explicitly and explain why, based only on the data given, "
        "rather than describing a yield or payment frequency that does not "
        "exist. Do not give a buy/sell recommendation."
    )
...
def report_writer_final_report(
    ...
    instruction = (
        "Write a final research report for GLD (SPDR Gold Shares, a "
        "physically-backed gold ETF) that integrates all sections below into "
        "one coherent document with clear headers (Composition, Holdings/"
        "Exposure, Cost/Tracking, Performance/Risk, Distribution, Macro, "
        "Bull Case, Bear Case, Synthesis). End with an explicit disclaimer "
        "that this is an analysis exercise, not investment advice or a trade "
        "recommendation."
    )
    payload = (
        f"[COMPOSITION]\n{composition}\n\n"
