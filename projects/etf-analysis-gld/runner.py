"""GLD ETF Analysis runner (네 번째 ETF 반복 실행, 최초의 원자재형 ETF).
...
Parser/Scheduler/Dispatcher를 만들지 않는다(QQQ/SCHD/AGG runner.py와 동일한
성격).

JPM/AGG와 동일하게 각 호출의 입력 길이/출력 길이/소요 시간을 `call_log.json`에
기록한다. 이는 새 Contract나 Capability가 아니라 project-local 관찰 데이터다.
...
ISSUE_DIR = PROJECT_ROOT / "issues" / "0001-gld-analysis"
...
_FUND_HEADER = "Fund: SPDR Gold Shares (Ticker: GLD)"
