...
def _cmd_edit(args, store):
    tags = None if args.tags is None else _parse_tags(args.tags)
    note = store.update(args.note_id, title=args.title, body=args.body, tags=tags)
    if note is None:
...
def build_parser():
    ...
    edit_parser = subparsers.add_parser("edit", help="Edit a note", parents=[sub_store_parent])
    edit_parser.add_argument("note_id")
    edit_parser.add_argument("--title", default=None)
    edit_parser.add_argument("--body", default=None)
    edit_parser.add_argument("--tags", default=None, help="Comma-separated tags")
