...
도메인은 Dev HQ도 Investment HQ도 아닌 제3의 중립 영역(조수·단풍·발효)이다 — `hqs/`·`core/`를 import하지 않는다.
"""
...
def run_exception_propagation_check() -> dict:
    """정상 Task 2개 + 실패하는 Task 1개(존재하지 않는 바이너리 호출 — 실제 subprocess 예외)를 함께 제출해 전파를 확인한다."""
...
def check_zero_dependency() -> list[str]:
    """이 디렉터리의 .py 파일들이 hqs/ 또는 core/를 import하지 않는지 소스 레벨로 정적 검사한다."""
    violations = []
