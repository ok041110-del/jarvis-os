"""PG Dividend Stock Analysis runner — 두 번째 Dividend Stock Dogfooding.
...
— Workflow Parser/Scheduler/Dispatcher를 만들지 않는다(JNJ runner.py와
동일한 성격, 코드는 공유하지 않음).

JNJ와 동일하게 각 호출의 입력 길이/출력 길이/소요 시간을 `call_log.json`에
기록한다.
