"""PG Dividend Stock Analysis runner — 3번째 Dividend Stock Dogfooding
(`docs/research/DIVIDEND-STOCK-DOGFOODING-REVIEW-0001.md` §7이 권고한
반복 실행, JNJ/KO에 이은 3회차 — Promotion 3회 기준 완성 목적).

raw_data.md(이 세션이 WebSearch로 미리 수집한 실제 데이터) -> 7개 전문 분석
(Fundamental/Dividend Quality/Valuation/Technical/Industry-Competition/
News-Event/Sentiment) -> Bull Case/Bear Case -> Synthesis -> Final Report.
각 단계는 `agents.py`의 함수를 순서대로 직접 호출하는 하드코딩된 흐름이다
— Workflow Parser/Scheduler/Dispatcher를 만들지 않는다(JNJ/KO/Stock/ETF
runner.py와 동일한 성격).

JNJ/KO와 동일하게 각 호출의 입력 길이/출력 길이/소요 시간을
`call_log.json`에 기록한다.
...
ISSUE_DIR = PROJECT_ROOT / "issues" / "0001-pg-analysis"
...
_COMPANY_HEADER = "Company: The Procter & Gamble Company (Ticker: PG)"
