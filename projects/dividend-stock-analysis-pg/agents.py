...
`projects/dividend-stock-analysis-jnj`와 코드를 공유하지 않는 별도의
project-local 구현이다(두 번째 Dividend Stock Dogfooding — Dividend
Stock Team/Agent를 이 파일에서 선행 설계하지 않는다).

JNJ와 동일하게 Stock의 5개 분석(Fundamental/Technical/
Industry-Competition/News-Event/Sentiment)과 최대한 동일한 지시문
패턴을 유지하고, Dividend Quality와 Valuation을 추가해 7개 분석 →
Bull/Bear → Synthesis → Final Report 구조를 쓴다 — 이 반복 자체가
"Stock 5개 역할이 다른 배당주에서도 재사용되는지" 검증 대상이다.
"""
...
def report_writer_final_report(
    ...
    instruction = (
        "Write a final research report for PG that integrates all sections "
