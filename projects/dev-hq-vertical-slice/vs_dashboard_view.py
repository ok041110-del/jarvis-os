"""Dashboard — Observe-only. `vs_pipeline`을 import하지 않는다(실행을 시작/재시도할 수단 자체가 이 모듈에 없다).

RUNNING 상태는 Task Registry(`rtb_task`, `runtime-boundary` Prototype 재사용)에서, 완료된 결과는 Result Store(파일)에서 읽는다 — 두 경로 모두 순수 조회다.
"""
