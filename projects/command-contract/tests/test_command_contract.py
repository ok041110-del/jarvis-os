...
def test_command_result_matches_dashboard_snapshot_content():
    """Command Layer가 자체 데이터를 만들지 않고 기존 Snapshot Builder를 그대로 재사용하는지 검증한다."""
...
def test_task_wrapping_adds_no_observable_value_for_single_readonly_command():
    """Case A/B를 같은 입력으로 비교해, 이 범위(단발 동기 read-only 명령)에서 Task가 실질적 이점을 제공하지 않음을 확인한다."""
...
def test_multi_hq_sequential_commands_do_not_require_shared_state():
    """이전 Command의 상태가 다음 Command에 영향을 주지 않는지 확인한다(Context 필요성 검증, Q4)."""
...
def test_command_has_no_id_field_and_resolution_still_works():
    """Command에 command_id가 없어도(Evidence 없이 필드 추가 금지) 요청-응답 흐름이 성립하는지 확인."""
