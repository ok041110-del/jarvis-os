...
설계: 함수 `development_hq_devkit(*args, **kwargs)`를 추가한다. 요구사항: 요구사항: 'Development HQ DevKit 최소 기능' 기능이 필요하다. 상세: Development HQ DevKit 프로젝트의 첫 기능: 실제 Issue 하나를 입력받아 Development HQ의 기존 Capability(Project Intelligence, Planning, Design, Validation)를 순서대로 실행하고, 그 결과를 planning.md, design.md, validation.md 세 개의 Markdown 파일로 저장한다. Implementation(Code Generation)은 이번 기능에 포함하지 않는다.
