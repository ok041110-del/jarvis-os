...
class TestWorkspace:
    """단일 실행에 대응하는 1회용 Workspace. `create()` -> `apply_implementation()` -> `run_pytest()` -> `cleanup()` 순서로 사용한다. 각 단계의 실패는 별도 예외/결과로 구분되며, 어떤 실패도 원본 repository에 영향을 주지 않는다(원본은애초에 이 클래스가 쓰기 대상으로 삼지 않는다)."""
    ...
    def apply_implementation(self, relative_target_path: str, implementation: str) -> None:
        """Workspace 내부(repository 밖 절대 없음) 대상 파일을 Implementation으로 덮어쓴다. `relative_target_path`는 repository root 기준 상대 경로여야 한다(예: `hqs/development/mvp/agents/backend.py`) — 이 밖으로 나가는 경로(`..` 포함)는 거부한다."""
    ...
    def run_pytest(self, relative_tests_dir: str, *, pytest_bin: str = "/root/.local/bin/pytest") -> dict:
        """Workspace 내부에서 pytest를 실행한다. 원본 repository는 참조하지 않는다(cwd·경로 전부 Workspace 내부로 고정). 실패(비정상 종료 포함)는 예외를 던지지 않고 구조화된 dict로 반환한다 — pytest FAIL과 실행 자체의 실패(예: 바이너리 없음)를 `executed` 필드로 구분한다."""
        if not self._created or self.workspace_root is None:
    ...
    def cleanup(self) -> WorkspaceCleanupResult:
        """Workspace 삭제. 실패해도 원본 repository는 절대 영향받지 않는다 (Workspace는 원본과 물리적으로 분리된 별도 경로였을 뿐이므로) — 이 메서드의 실패는 디스크 공간 누수일 뿐이라는 것을 호출자가 확인할 수 있도록 결과를 구조화해 반환한다."""
