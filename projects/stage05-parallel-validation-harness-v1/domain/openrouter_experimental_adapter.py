"""OpenRouter Experimental Validation Adapter — Production Stage Engine Routing과 무관하다. `engine_call: Callable[[str], str]` Contract를 만족하는 어댑터 하나만 제공한다(새 Gateway/Router 없음).

API Key/Credential은 이 파일이 다루지 않는다 — 세션 Egress Proxy가 `openrouter.ai` 요청에 자동으로 주입하므로 Authorization 헤더를 직접 설정하지 않는다."""
...
class OpenRouterCallError(RuntimeError):
    """단일 예외 타입만 노출한다(기존 Engine Adapter Contract와 동일 패턴)."""


def make_openrouter_engine_call(model: str, *, max_tokens: int = 1500, timeout: int = 90):
    """`model`은 호출자가 명시적으로 고정한다 — 자동 `openrouter/free` 라우팅을 쓰지 않는다."""
