"""Review Validator — 3개 Mode(disabled/deterministic/llm). 다른 5개 Validator 결과는 입력으로 받지 않으며(RFC-0039 §2.6.1), Aggregator에는 항상 advisory로만 전달된다.

LLM Mode는 `engine_call: Callable[[str], str]`을 주입받을 뿐 Engine을 직접 소유·선택하지 않는다 — 주입되지 않으면 `ReviewEngineNotConfigured`로 명시적으로 실패한다(조용한 스킵 금지)."""
...
def _build_llm_review_prompt(ctx: ValidationContext) -> str:
    """Review Input 경계 — Stage 03 Design/Stage 04 Implementation/Contract/Scope Context/Immutable Source Snapshot만 포함한다. 다른 Validator의 ValidatorResult는 인자로 받지 않아 구조적으로 섞일 수 없다."""
    instruction = (
