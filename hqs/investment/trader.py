...
TRADER_DECISION_INSTRUCTION = (
    "\n\n추가로, 여기 주어진 정보만을 근거로 이 개별 증권에 대한 현재 "
    "시점의 방향성 입장을 결정하라. Portfolio 맥락, 기존 보유 포지션, "
    "자본 배분, 포지션 규모는 가정하지 마라 — 판단을 완성하는 데 그런 "
    "정보가 필요하다면 추측하거나 지어내지 말고 명시적으로 그렇다고 "
    "말하라. 당신의 범위는 이 단일 증권의 현재 정보 기준 방향성 입장으로 "
    "한정된다 — 당신은 Portfolio Manager가 아니며 Portfolio 수준의 "
    "판단을 시도해서는 안 된다.\n\n"
    "다음 정확한 헤더를 사용해 명확히 분리된 두 섹션으로만 답하라:\n\n"
    "## REPORT\n"
    "사람 독자에게 필요한 모든 것: 위에서 설명한 것과 동일한 종합(사실 "
    "대 해석, 충돌 지점, 미해결 질문)을 중립적이고 방향성 없는 산문으로 "
    "작성하라. 이 섹션 어디에도 방향 판단을 두지 마라 — buy/sell/hold "
    "지시가 전혀 없는 독립적 분석으로 읽혀야 한다.\n\n"
    "## DECISION\n"
    "- Direction: BUY / SELL / HOLD 중 정확히 하나를 선택하라\n"
    "- Rationale: 2-4문장, 오직 위 REPORT 섹션에만 근거하고 새로운 "
    "사실을 추가하지 마라\n"
    "- Reassess when: REPORT에서 해소될 경우 이 Direction을 가장 크게 "
    "바꿀 가능성이 있는, 판단에 가장 결정적인 미해결 질문 하나를 명시하라"
)
