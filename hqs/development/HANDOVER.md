...
| Development HQ v2.0(RFC-0007 AST Context → ADC-0005 → ADR-0008 → Stage 01~05 → Integrated Workflow → CLI) | **종료 확정 — Stable v2.0 Freeze**(`DEVELOPMENT-HQ-V2.0-FREEZE-0001.md`로 승인·확정). Agent Layer Readiness Review + Refactoring Audit + Documentation/Docstring Refactoring 전부 완료, real Engine E2E 전 구간(Stage 01→05 + `workflow.py` + `cli.py`) PASS — `hqs/development/stages/`, `hqs/development/workflow.py`, `hqs/development/cli.py`, Evidence는 `docs/research/DEV-HQ-V2.0-*`. Freeze 이후 Agent Definition 0001(`DEV-HQ-V2.0-AGENT-DEFINITION-0001.md`, Requirements/Design/Backend/QA Agent 4개 확정) → RFC-0008 → ADC-0006(AST Context dotted package path additive extension, Conditional Accept) → Agent Package Refactoring(`hqs/development/mvp/agents.py` → `hqs/development/mvp/agents/{requirements,design,backend,qa}.py`)이 PR #103으로 main에 Merge 완료됨 — 테스트 기준선 **120 passed**, real Engine E2E PASS(`DEV-HQ-V2.0-AGENT-PACKAGE-REFACTORING-E2E-0001.md`), Architecture/Contract 변경 NONE. 이후 Stage 01~05 Data Contract 정리(`hqs/development/stages/contracts.py` 신규, Stage 04/05 미사용 파라미터 제거, `candidate_index` 중복 계산 제거, Stage 05 `required_checks`/`check_results` 구조화)가 진행되어 테스트 기준선 **143 passed**, real Engine E2E 2건 PASS — `DEV-HQ-V2.0-STAGE-DATA-CONTRACT-0001.md`. 실사용 검증에서 그 `required_checks`가 실행/Verdict와 인과관계 없는 decorative mirror임이 드러나, `_CHECK_EVALUATORS` 공유 표 + `contracts.validate_verification_requirement/result()`로 실제 인과관계(실행 집합 결정·SKIPPED 기록·Verdict 반영·빈 값/미지 이름 즉시 거부)를 연결 — 테스트 기준선 **152 passed**, real Engine E2E로 `required_checks` 변경이 Verdict를 실제로 바꾸는 것 확인, Security/Data-API는 대응 Capability 부재로 여전히 미해결로 명시 — `DEV-HQ-V2.0-STAGE-DATA-CONTRACT-0002.md`. Stage 개수/순서/Capability/Agent 무변경, 새 RFC/ADC/ADR 없음. v1.0 Freeze와 별개의 추가 트랙(Baseline 미변경). Production 진입 Blocking·Kernel Open Decision과는 별개(그쪽은 계속 Open) |
| Development HQ Team Architecture Migration v1 | Stage 01~05를 Context/Planning/Architecture/Implementation/Validation Team 구조로 Migration 완료(`hqs/development/teams/`). 각 Team은 대응하는 기존 `stages/0N_*/stage_0N.py` capability를 그대로 호출만 하며 재구현하지 않음, `workflow.py`가 Team 순서 호출로 전환됨(Handover semantics·`stages/contracts.py`의 5개 Contract 무변경). Meta Architecture가 이미 허용한 "Division/Team = HQ 내부 선택 구조"(HANDOVER.md Frozen Architecture) 범위 안의 구조 정리이며, Kernel/Public Contract/Agent Domain 변경 아님. `stages/`는 `mvp/tests/test_stage_0N.py` 등이 여전히 직접 참조해 제거하지 않음(§11 판단, 보존). 테스트 기준선 **186 passed, 6 skipped 무변경**(회귀 없음, Team Architecture Migration v1 시점 기준값 — 이후 다른 작업으로 계속 증가했으며 아래 "테스트 기준선(현재)" 행 참고). Agent 신규 구현 없음 — Team 내부 향후 Agent 후보는 `teams/README.md`에 기록만 하고 미착수(RFC-0030 Proposed 분석 기준) |
| OmniRoute Model Routing/Engine Adapter | **Production Adoption 완료 — Scoped(Case A: Thin Engine Caller)**. RFC-0023 → ADC-0027~0031(Conditional·Scoped Accept) → ADR-0015~0016~0017(Final Adoption Review, Production Adoption 선언, Freeze §Scoped 예외)로 OmniRoute 채택 — Jarvis 코드는 단일 함수(`hqs/development/mvp/omniroute_engine.py::call_engine_via_omniroute`) 하나만 갖고 Provider/Model 선택·Routing·Policy 판정 로직을 두지 않는다. 실제 운영 `~/.omniroute` 인스턴스에 `blockedProviders`(NOAUTH 12종)/`REQUIRE_API_KEY` 안전 설정을 적용·실증(`EVIDENCE-0012`). 기존 5개 call-site(`agents/{backend,design,qa,requirements}.py`, `workflow_ast_context.py`)를 Claude CLI → `call_engine_via_omniroute`로 동기화 전환 완료 — **PR #159, main merge commit `a1bfd81`**. RT-0001 Candidate 2("호출 지점 ≥2")는 전환 후 기존 call_engine() 참조 0개(교체이지 추가 아님)로 non-trigger, ADC-0010 C1(Engine Caller 위치, Not Accepted)은 이 전환으로 재고되지 않고 그대로 Open 유지 — Case A 범위를 벗어나는 구현(Engine Gateway/Jarvis 자체 Routing·Policy/Multi-Engine)은 여전히 Freeze 대상. `engine.py`(옛 Claude CLI caller) 자체는 무변경으로 보존. Evidence: `docs/architecture/core/EVIDENCE-0001`~`EVIDENCE-0013` |
| Dashboard Shell MVP(`projects/dashboard-shell-mvp/`) | Development HQ MVP 범위 밖의 **Experimental Prototype**(Formal Architecture Decision 아님, `projects/dashboard-shell-mvp/README.md` 참조, Production `dashboard/`는 여전히 Deferred). Chat 입력을 실제 `claude -p`(`--tools ""`로 Tool 전부 비활성화, 분류 전용 1회 호출)로 intent/target_hq 해석 → 기존 `projects/command-contract/resolver.py`의 `resolve()`에 그대로 연결(새 Command Contract 생성 없음) — **PR #157, main merge commit `b6a9fdf`**. Trading은 Claude가 분류해도 기존 resolver가 `unknown_hq`로 그대로 거부(신규 routing/policy 없음 실증), Claude 호출 실패/timeout/schema 위반은 Mock 대체 없이 502로 노출. `core/`/`hqs/`/Production `dashboard/`/OmniRoute 코드 무변경 |
| 테스트 기준선(현재) | `hqs/development/mvp/tests/`: **364 passed, 6 skipped** — PR #209(Issue #206 후속 문서 정정) 검증 시점 실측값이며, RFC-0007 Role Boundary 후속 작업 세션에서 재실행해 재확인함(이 표의 위 행들에 남아있는 120/143/152/186 passed는 각 시점의 역사적 기준값이며 소급 수정하지 않음). `core/execution/`: **55 passed**(별도 스위트, `hqs/development/mvp/tests/`와 합산하지 않음) |
