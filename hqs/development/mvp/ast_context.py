"""ADC-0005 §1/§2 — AST Function Candidate Index / Dependency Closure, Engine 미호출 순수 정적 분석(RFC-0007 §4).
...
def module_source_path(module: str) -> Path:
    """`module` 이름을 소스 경로로 변환한다(Target File Exposure 배선용, ADC-0005 §7).
    dotted 이름(`pkg.sub`)은 `pkg/sub.py`로 변환한다(ADC-0006 additive extension)."""
...
def _candidate_entries_from_source(source: str, filename: str = "<source>"):
    """로컬 파일용 `_candidate_entries()`가 감싸는 핵심 로직 — Snapshot 기반(GitHub) 입력도
    이 함수를 그대로 재사용한다(Stage 01 Multi-Agent Code Analysis, RFC-0033 §9 최소 수정 원칙)."""
    try:
...
def build_function_candidate_index() -> str:
    """저장소 함수 후보를 이름+시그니처+docstring 첫 줄로 색인화한다(RFC-0007 §2 시작점 식별용).
    패키지 디렉터리가 있으면 평면 파일 목록 뒤에 이어 붙인다(ADC-0006 additive extension)."""
    sections = []
