...
# Exposure Policy 충돌 시 Engine의 결정적 신호(`_EXPOSURE_POLICY_INSTRUCTION`,
# ADC-0005 §8 검증됨, Stage 04는 수정하지 않음)에 이 문구를 이어붙인다(Phase 2.5 Case C).
...
def run_stage_04(stage_01_context: dict, stage_03_output: dict, expose_target: bool = False) -> dict:
    """Target Identification(Stage 01 `candidate_index` 재사용, 재계산 없음) -> Closure/Exposure 조립 -> Code Generation. `issue`는 이 Stage가 실제로 쓰지 않아 Input에서 제거했다(CandidateIndex Contract, Producer: Stage 01 / Consumer: Stage 04)."""
