...
§4). 세 실험군 모두 동일한 `build_input`(동일 Design/Target/Context,
§5)을 입력받는다 — Target Identification 자체의 변동성은 이 Harness의
비교 대상이 아니다.
...
from deterministic_checks import run_deterministic_gate  # noqa: E402
...
def _gate(code: str, allowed_function_names: tuple) -> dict:
    return run_deterministic_gate(code, required_keys=STAGE_04_CONTRACT_KEYS, allowed_function_names=allowed_function_names)


def run_variant_a(case_id: str, build_input: str, engine_call, allowed_function_names: tuple) -> tuple:
    ...
    gate_result, validation_ms = _timed(_gate, code, allowed_function_names)
    ...
    return result, (code if gate_result["passed"] else None)


def _generate_and_gate_candidates(build_input: str, engine_call, allowed_function_names: tuple) -> tuple:
    ...
    def _gate_all():
        for candidate in candidates:
            candidate["gate"] = _gate(candidate["code"], allowed_function_names)
        return candidates
...
def run_variant_b(case_id: str, build_input: str, engine_call, allowed_function_names: tuple) -> tuple:
    ...
    candidates, generation_ms_total, validation_ms = _generate_and_gate_candidates(
        build_input, engine_call, allowed_function_names
    )
    ...
    return result, (best["code"] if best else None)


def run_variant_c(case_id: str, build_input: str, engine_call, allowed_function_names: tuple) -> tuple:
    ...
    return result, (final["code"] if final else None)
