"""Stage 04 Architecture Validation — Test Cases(RFC 요청 §12). 5개 대표 Case를 고정된 `build_input`(Design 역할), `target`, `expose_target`으로 정의한다 — 전부 합성 데이터라 실제 repository 파일을 참조하거나 변경하지 않는다. `target`/`expose_target`을 Case에 고정해 A/B/C 전부 동일한 값을 쓰게 한다(Controlled Variables — Target Resolver 최적화는 별도 후속 실험).

`allowed_function_names`(정의해도 되는 최대 범위)와 `required_function_names`(반드시 정의해야 하는 최소 범위)를 구분한다 — `case_c_helper_reuse`처럼 "이미 존재하는 helper는 재사용만 하고 다시 정의하지 않아야 한다"는 요구를 표현하려면 둘이 달라야 한다.
"""
