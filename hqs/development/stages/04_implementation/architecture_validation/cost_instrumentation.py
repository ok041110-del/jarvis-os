...
`mvp/omniroute_engine.py::call_engine_via_omniroute()`는 응답 텍스트만 반환하고 OpenAI-compatible 응답의 `usage`(prompt/completion/total tokens) 필드를 그대로 버린다 — Thin Caller Contract(`str -> str`, ADC-0031/ADR-0017)를 그대로 유지하기 위해 의도된 설계이지, 버그가 아니다. 이 Harness는 그 Production Contract를 바꾸지 않는다
"""
...
def call_with_usage(prompt: str) -> tuple:
    """`call_engine_via_omniroute()`와 동일한 요청을 보내되, 응답 텍스트와 `usage` dict(없으면 `None`)를 함께 반환한다. Production 함수를 감싸거나 대체하지 않는다 — 완전히 별도 경로다(§7: Thin Caller Contract 임의 변경 금지)."""
...
def usage_to_result_fields(usage: dict) -> dict:
    """OpenAI-compatible `usage` dict를 Result Schema 필드명으로 옮긴다. `usage`가 `None`이면 전부 `None`(미확보)을 유지한다 — 0으로 치환하지 않는다."""
    if not usage:
