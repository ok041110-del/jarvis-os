"""Stage 01 Code Analysis Executors(RFC-0033 §8/§9) — 기존 mvp의 순수 로직을
재사용하고 local filesystem 순회만 GitHub Snapshot 기반으로 바꾼다."""
...
def structure_analysis(snapshot) -> list:
    """GitHub Tree API가 이미 완전한 목록을 주므로 별도 순회 알고리즘이 필요 없다."""
...
def relevant_discovery(snapshot, fetch_content, issue: dict, search_specification: dict) -> dict:
    """GitHub API 호출 비용 억제를 위해 파일명으로 1차 후보를 추린 뒤
    상위 후보만 content를 fetch해 채점한다(RFC-0033 §9)."""
...
def dependency_analysis(target) -> str:
    """§8 Conditional — `target`이 주어질 때만 실행한다. Stage 01은 실제로 target을
    받은 적이 없어 Snapshot 기반 재구현은 보류한 상태다(Known Limitation)."""
