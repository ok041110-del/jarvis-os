...
- **Prompt/String Literal**: Engine에 전달되는 프롬프트 등 사람이 읽는 여러 줄 문자열도 기본 한글로 작성한다. 단, 파싱/매칭이 의존하는 리터럴 토큰(헤더, 필드명, enum 값 등)은 원문(영문) 그대로 유지한다. 문자열을 여러 줄로 나눠 쓸 때는 줄바꿈 지점이 단어를 자르지 않게 한다 — 각 줄 끝에 공백을 둬 결합 결과가 매끄럽게 이어지도록 하고, 가능하면 문장·구 경계에서 끊어 소스코드 자체도 자연스럽게 읽히게 한다.
