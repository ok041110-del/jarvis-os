...
Architecture 자체에 대한 설명은 이 문서에서 반복하지 않는다. `docs/architecture/baseline/BASELINE.md`가 유일한 Architecture 원본이다.
...
| Execution Layer MVP | MVP-0001 ~ MVP-0006 완료 (`core/execution/`) |
...
Architecture Baseline과 Development HQ Baseline은 직접 수정하지 않는다. 변경이 필요하다고 판단되면 `docs/decisions/rfc` → `docs/decisions/adc` → `docs/decisions/adr` 절차를 따른다.

## 시작 위치

Claude Code는 `hqs/development/HANDOVER.md`를 가장 먼저 읽는다.
...
│   ├── architecture/baseline/    BASELINE.md — Jarvis OS Architecture 원본
│   ├── 01_mvp/                   Development HQ MVP별 plan/observation
│   ├── decisions/
│   │   ├── rfc/                  RFC (Architecture 변경 제안)
│   │   ├── adc/                  ADC.md — Open Decision Single Source of Truth
│   │   └── adr/                  ADR (채택된 결정)
...
├── core/execution/                Execution Layer MVP 코드(ADR-0006 Phase 1: core/execution_layer/에서 이동)
├── hqs/
│   ├── development/               (ADR-0006 Phase 2: development-hq/에서 이동)
│   │   ├── README.md, MISSION.md, BOUNDARY.md, RESPONSIBILITY.md, STRUCTURE.md
│   │   ├── BASELINE.md, MVP.md, IMPLEMENTATION_RULES.md, HANDOVER.md
│   │   └── mvp/                   MVP-0001 구현 코드
│   └── investment/                Investment HQ(Stock/ETF/Dividend Stock Team) 실행 코드(ADR-0006 Phase 2: investment-hq/에서 이동)
...
1. `README.md` (본 문서)
2. `hqs/development/HANDOVER.md` — 인수인계 요약, 시작점
3. `docs/architecture/baseline/BASELINE.md` — Jarvis OS 전체 기준
4. `hqs/development/BASELINE.md` — Development HQ 기준
5. `hqs/development/MVP.md` — MVP-0001이 무엇을 만들었는가
6. `hqs/development/IMPLEMENTATION_RULES.md` — 무엇을 만들면 안 되는가
7. `docs/decisions/adc/ADC.md` — 아직 결정되지 않은 것들 (구현과 무관, 참고용)
